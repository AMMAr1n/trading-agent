...
class TradingExecutor:
    ...
    async def execute_decision(
        ...
        if volume_ratio < 0.5:
            ...
            logger.info(f"Volumen muy bajo ({volume_ratio:.1f}x) — monto reducido al 15%: ${decision.amount_usd:.2f}")
        elif volume_ratio < 0.8:
            decision.amount_usd = min(decision.amount_usd, balance.operable * 0.20)
            logger.info(f"Volumen bajo ({volume_ratio:.1f}x) — monto reducido al 20%: ${decision.amount_usd:.2f}")
        ...
        logger.warning(
            ...
            f"(capital operable: ${balance.operable:.2f}, volumen: {volume_ratio:.1f}x)"
        )
    ...
    async def _execute_autonomous(
        self,
        decision: TradeDecision,
        balance: BalanceInfo          # ← recibe balance
    ) -> Optional[OrderResult]:
        logger.info(
        ...
        # Notificar entrada — ahora con saldo real
        if self.notifications_enabled and os.getenv("NOTIFY_ON_ENTRY", "true") == "true":
            self.notifier.notify_trade_opened(
                ...
                account_balance=balance.usdt_free,   # ← FIX: saldo real
        ...
        if result.success:
            ...
            # Si el SL/TP falló, avisar al operador
            if result.error_msg:
                if self.notifications_enabled:
                    self.notifier.notify_critical_error(
                        f"⚠️ {decision.symbol}: posición abierta pero SL/TP NO se colocó. "
                        f"Ponlo manualmente en Binance ahora."
                    )
        else:
    ...
    async def _execute_with_vobo(
        self,
        decision: TradeDecision,
        balance: BalanceInfo          # ← recibe balance (por consistencia)
    ) -> Optional[OrderResult]:
        logger.info(
